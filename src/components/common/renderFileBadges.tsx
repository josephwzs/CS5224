import { File, FileCode2, FileImage, FileSpreadsheet, FileText, Presentation } from 'lucide-react';
...
export function renderFileBadges(fileKeys: string[], accessToken?: string, downloadFile2?: (params: any) => void) {
  return fileKeys.map((key, i) => {
    ...
    return (
      <Badge
        variant="outline"
        key={i}
        onClick={downloadFile2 ? () => downloadFile2?.({ fileKey: key }) : () => downloadFile(key, accessToken)}
